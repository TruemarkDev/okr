class HomeController < ApplicationController
  ...
  def dashboard
    ...
    @entry_hash={}
    #entries = Task.where('start_date <= ? AND end_date >= ?',@end_date.end_of_day,@start_date.beginning_of_day)
  ...
  def search
    ...
    if @task.present?
      ...
      @tasks = Task.searchable_for_user(current_user).ransack('tracker_id_or_name_or_description_cont'=>params[:search][:keyword]).result.paginate(page: params[:page], per_page: 10).order('id DESC')
